...
//		}
	...
	public void setBigblind(double bigblind) throws HandIllegalBigblindException {
		...
		if (bigblind == 0.0) {
			throw new HandIllegalBigblindException(Messages.Table_4);
		}
	...
	public void setPotExtern(double value) throws IllegalArgumentException {
		...
		if (value < 0) {
			...
			throw new IllegalArgumentException(Messages.Table_6);
		}
	...
	public void addPlayer(Player player, int position)
		...
		if (player == null) {
			throw new PlayerMissingException(Messages.Table_7);
		}
	...
	public String toString() {
		...
		case POKERSTARS:
			...
			break;
		default:
			ret = Messages.Table_9;
			break;
		}
		
		// Spielnummer
		ret = ret  + " " + game + //$NON-NLS-1$
		      "\n"; //$NON-NLS-1$
...
//		df.format( dt );
		...
		// max. Anzahl Plaetze
		ret = ret + String.valueOf(countSeats) + Messages.Table_12;
		...
		for (int i = 0; i < countSeats; i++) {
			ret = ret + 
			      ...
			      ": "; //$NON-NLS-1$
			if (players[i] == null) {
				ret = ret + Messages.Table_15;
			} else {
				ret = ret + players[i].getName() + " (" //$NON-NLS-1$ 
						+ players[i].getChips() + Messages.Table_16;
				
				// setzt Spieler aus
				if (players[i].getState() == Player.SITTINGOUT) {
					ret = ret + Messages.Table_17;
				}
			}
	...
	public int addBoardCard(Card card) throws CardException, HandIllegalCardCountException {
		...
		if (card == null) {
			throw new CardException(Messages.Table_20);
		}
		...
		if (cardCount > 4) {
			throw new HandIllegalCardCountException(Messages.Table_21);
		}
	...
	public Table copy() {
		...
		try {
			...
			ErrorHandler.handleError(e, Messages.Table_28, false);
	...
	public double getBetOfPlayer(int pos) throws IllegalArgumentException {
		...
		if (pos >= bet.length) {
			...
			throw new IllegalArgumentException(Messages.Table_30);
		}
	...
	public double payOut(double price) throws TableException {
		...
		if (pot[potNumber] < price) {
			throw new TableException(Messages.Table_31);
		}
			...
			for (int i = 0; i < bet.length; i++) {
				if (bet[i] != 0) {
					try {
						...
						ErrorHandler.handleError(e, Messages.Table_32, false);
					}
				}
			}
		}
